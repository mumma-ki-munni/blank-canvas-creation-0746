...
export function UserAvatar({
  ...
  /**
   * This app stores a colour per profile, so pass it in and everyone keeps the
   * same colour here as in the sharing list. Falls back to one worked out from
   * the id, which is stable for the same reason.
   */
  ...
  const derived = avatarColors(userId);
  const background = colorOverride ?? derived.background;
  const color = colorOverride ? "#fff" : derived.color;
